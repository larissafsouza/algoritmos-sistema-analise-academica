...
public class FiltraPorPerfil {
	...
	public FiltraPorPerfil(String curso){
		current_curso = curso;
			alunos = null;
				try {
	...
	public List<String> filterByJubilado(String forma_saida, double porcentagemConjunto){
			...
			List<String> listRestante = new ArrayList<String>();
			...
			try {
				...
				"training_set"+forma_saida.toLowerCase().trim().replace(" ", "").replace("(crit.01)", "")+"_"+current_curso.toLowerCase().trim().replace(" ", "_"));
			} catch (UnknownHostException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			size = (int)((double)size * porcentagemConjunto);
			System.out.println(size);
...
//						trainingSet.save(document);	
					...
					listRestante.add(id_aluno);
				}
				count ++;
			}
			
			list_excluidos_perfil = listRestante;
			
			System.out.println("LISTA DE EXCLUIDOS DO PERFIL");
			System.out.println(listRestante);
...
//			System.out.println("dbo: " +dbo);
	...
	public void getAverageDisciplines(String forma_saida) {
		...
		try {
			...
			"vector_courses_training_set"+forma_saida.toLowerCase().trim().replace(" ", "").replace("(crit.01)", "")+"_"+current_curso.toLowerCase().trim().replace(" ", "_"));
		} catch (UnknownHostException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
//			System.out.println("dbo: " +dbo);
	...
	public void getAverageDisciplinesGeral(String forma_saida) {
		...
		try {
			vectorCourses = MongoConnection.getInstance().getDB().getCollection(
			"vector_courses_geral"+forma_saida.toLowerCase().trim().replace(" ", "").replace("(crit.01)", "")+"_"+current_curso.toLowerCase().trim().replace(" ", "_"));
		} catch (UnknownHostException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
//			}
	...
	public List<List<String>> getResultadosExperimentosDistanceDoPerfil(String forma_saida, String id_representante, double distMax) {
		...
		System.out.println(listDistance.size());
		return listDistance;
	}
	...
	public static void main(String args[]) {
		FiltraPorPerfil ativ= new FiltraPorPerfil("ciencia da computacao");
...
//		ativ.getAverageDisciplines("jubilado (crit. 01)");
		...
		// SELECIONADAS DENTRO DO CONJUNTO DE DISCIPLINAS FEITAS PELOS ALUNOS DENTRO DO CONJUNTO
//		ativ.getAverageDisciplinesGeral("jubilado (crit. 01)");
